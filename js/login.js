...
const Auth = {
  ...
  logout() {
    Store.remove('bookly_currentUser');
...
function initDarkMode() {
  const saved = Store.get('bookly_darkMode', false);
...
// ===== BUILD NAVBAR (sempre mostra Entrar/Cadastrar pois não está logado) =====
function buildNavbar() {
  ...
  <img src="logo.png" alt="Bookly" style="height:80px;width:auto;object-fit:contain;">
    ...
    <div class="nav-actions">
      <button class="nav-btn-icon" onclick="toggleDarkMode()" title="Modo claro/escuro">${darkIcon}</button>
  ...
  document.getElementById('mobile-menu').innerHTML = `
    <div class="mobile-menu-header">
      <a href="index.html" class="logo">
        <div class="logo-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></div>
        <span>Bookly</span>
      </a>
      <button class="mobile-menu-close" onclick="closeMobileMenu()"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
...
function handleLogin(e) {
  ...
  if (!user) {
    errText.textContent = 'E-mail ou senha incorretos. Verifique seus dados.';
    errEl.classList.add('show');
    return false;
  }

  Store.set('bookly_currentUser', { id: user.id, name: user.name, email: user.email, plan: user.plan || null });
